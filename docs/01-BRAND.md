...
Source of truth for who NUCLIEX is and how it speaks. Everything on the site must be traceable to
this file or to `05-CONTENT.md`.
...
founder, a real address, and downloadable documentation do that. Gradients and buzzwords do not.
...
3. **Short sentences.** If a sentence needs a semicolon, it probably needs to be two sentences.
4. **Name things as the customer names them.** "SSD upgrade for your laptop", not "storage subsystem migration."
5. **Never oversell warranty or support.** State the exact term. Ambiguity here is what customers
   are scanning for.
6. **Numbers are always specific and always sourced.** No "up to 10x faster" without the comparison stated.
7. **Indian English conventions:** ₹ for currency, lakh/crore only in business contexts, DD Month YYYY
...
`next-generation` is permitted only when it literally means a PCIe generation.
...
Annual Maintenance Contracts (AMC) · remote technical support · system optimisation ·
...
> **Rule:** the services are the revenue today; the products are the ambition. The site must sell
> both without letting the roadmap read as if it already ships. Anything not currently purchasable
> carries a visible `Planned` or `Coming soon` badge.

---

## 6. Trust assets to surface prominently

These are the site's real differentiators. Give them page-level or section-level prominence:

1. **Warranty terms**, stated as a number of years, with the claim process in plain steps.
2. **RMA / replacement process** — a numbered, honest sequence with expected timelines.
3. **Named founder and a real address** — a face and a place defeat the "shell brand" suspicion.
4. **Downloadable documentation** — datasheets, spec sheets, install guides.
5. **Support channels with stated response times.**
6. **GST-compliant invoicing** for business buyers.
7. Certifications — **only once verified** (`09-OPEN-QUESTIONS.md`).
...
- **Never:** stretch, rotate, add shadow/glow/outline, place on a busy image, recolour outside the
  approved set, or reintroduce the original teal.
- Header uses the full lockup at ~150–170px wide desktop, mark + wordmark reduced or mark-only ≤ 640px.
- The node-network mark may be reused as a **very low-opacity background motif** (≤ 4% on dark
  sections only) — this is the one permitted decorative use. Never at higher opacity, never animated.
