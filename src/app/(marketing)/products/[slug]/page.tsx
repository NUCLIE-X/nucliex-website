...
export default async function ProductPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const breadcrumbJsonLd = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 2, name: "Products", item: `${env.NEXT_PUBLIC_SITE_URL}/products` },
  ...
  return (
    <>
      ...
      <Section spacing="tight" className="pt-16 md:pt-20">
        ...
        <div className="grid gap-10 lg:grid-cols-2 lg:gap-16">
          ...
          <div>
            ...
            <p className="text-body-lg text-fg-muted mt-3 max-w-[56ch]">{product.tagline}</p>

            {product.capacities.length > 0 ? (
              <CapacitySelector capacities={product.capacities} className="mt-8" />
            ) : (
              <p className="text-body-sm text-fg-subtle mt-8">
            ...
            <div className="mt-8 flex flex-wrap gap-4">
              ...
              {datasheet ? (
                <Button href={datasheet.href} variant="secondary" icon={Download}>
                  Download datasheet
                </Button>
              ) : null}
            </div>

            <p className="text-body-sm text-fg-muted border-border mt-8 border-t pt-4">
              Warranty:{" "}
      ...
      <Section spacing="tight">
        <h2 className="text-h2 font-display text-brand-900 font-medium">Overview</h2>
        <p className="text-body text-fg-muted mt-4 max-w-[68ch]">{product.summary}</p>
      </Section>

      <Section spacing="tight">
        <h2 className="text-h2 font-display text-brand-900 font-medium">Specifications</h2>
        {product.specs.length > 0 ? (
          <SpecTable specs={product.specs} className="mt-6 max-w-2xl" caption={`${product.name} specifications`} />
        ) : (
          <div className="border-border mt-6 max-w-2xl rounded-lg border border-dashed p-6">
            <p className="text-body text-fg-muted">
              {"{{TBD:specs}}"} — full specifications, including the conditions
              they were measured under, are published once validation
              completes. We don&rsquo;t print numbers we haven&rsquo;t verified.
            </p>
          </div>
        )}
      </Section>
      ...
      <Section spacing="tight">
        <h2 className="text-h2 font-display text-brand-900 font-medium">What it&rsquo;s for</h2>
        <ul className="mt-6 grid gap-6 sm:grid-cols-3">
          {product.useCases.map((useCase) => (
            <li key={useCase} className="border-border border-t pt-4">
              <p className="text-body text-fg font-medium">{useCase}</p>
            </li>
          ))}
        </ul>
      </Section>
      ...
      <Section spacing="tight">
        <h2 className="text-h2 font-display text-brand-900 font-medium">Warranty &amp; support</h2>
        <p className="text-body text-fg-muted mt-4 max-w-[68ch]">
          Register once and the warranty follows the drive. If it fails inside
          the term, the RMA process is a numbered sequence with stated
          timelines — not a negotiation.
        </p>
      ...
      {product.documents.length > 0 ? (
        <Section spacing="tight">
          <h2 className="text-h2 font-display text-brand-900 font-medium">Documents</h2>
          <ul className="mt-6 space-y-3">
            {product.documents.map((doc) => (
              <li key={doc.href}>
                <a href={doc.href} className="text-body text-brand-500 hover:text-brand-600 font-medium">
                  {doc.title}
                </a>
              </li>
            ))}
